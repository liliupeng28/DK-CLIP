...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        ...
        return a, b.mean(dim=1)

    def forward(self, x: torch.Tensor):
        v, a = self.attention(self.ln_1(x))
        x = x + v
        x = x + self.mlp(self.ln_2(x))
        return x, a


class MultisnippetIntegrationTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(1, 0, 2)
        x, atts = self.resblocks(x)
        x = x.permute(1, 0, 2)
        x = x.type(ori_x.dtype) + ori_x

        return x.mean(dim=1, keepdim=False), atts
