...
class DecordInit:
    ...
    def __call__(self, results):
        ...
        if results['tar'] is False:
            ...
            file_obj = glob.glob(os.path.join(results['filename'], '*.jpg'))
            # file_obj = glob.glob(os.path.join(results['filename'], '*.png'))
            file_obj.sort()
        else:
    ...
    def __call__(self, results):
        ...
        for idx in frame_inds:
            if idx >= len(container):
                idx = len(container) - 1
                imgs.append(np.array(Image.open(os.path.join(container[idx]))))
            elif idx < 0:
                idx = 0
                imgs.append(np.array(Image.open(os.path.join(container[idx]))))
            else:
