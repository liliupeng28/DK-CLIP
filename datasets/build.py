        ...
        with open(self.ann_file, 'r') as fin:
            for line in fin:
                ...
                if self.multi_class:
                    ...
                    if len(line_split) > 3:
                        mid = line_split[0] + ' ' + line_split[1]
                        filename, num_frames, label = [mid, line_split[2], line_split[3]]
                        label = int(label)
                    else:
                ...
                if int(num_frames) >= 3:
                    video_infos.append(
                        dict(filename=filename, label=label, num_frames=num_frames, tar=self.use_tar_format))
        return video_infos
